...
- 判定用的是**真实客户端地址**：默认取 TCP 连接的对端地址；部署在反向代理之后时，只有「对端属于 `server.trustedProxies`」的请求才按 `X-Forwarded-For` 还原客户端 —— **只取最右一项**（= 我方代理用 `$proxy_add_x_forwarded_for` 追加的那一跳，也就是它亲眼看到的对端），**不向左回溯**。回溯看似能穿过多级代理，但最右项本身落在可信网段内时（多级代理，或 Docker 把宿主机来源改写成网桥网关 `172.x.0.1`），继续往左就进入客户端自带的、可伪造的那一段 —— 实测过「伪造 `X-Forwarded-For: 10.99.9.9` 即可通过只放行 `10.99.9.9/32` 的白名单」，因此宁可粗一点也不回溯。其他来源发来的 XFF 一律忽略；IPv4-mapped 地址（`::ffff:1.2.3.4`）会归一化成 IPv4 再比较；
...
**摘要链路（写操作）同样受这份白名单约束**：`server.digestAllowlistEnabled`（默认 `true`）为真时，所有写接口的鉴权**先看来源网段、再验摘要** —— 名单外的来源即便摘要有效也返回 `403 forbidden`（`当前来源地址不在白名单内（server.permissionAllowlist），写操作被拒绝`），并留一条 `写操作被拦下：来源不在白名单内` 的 `warn` 日志（字段与权限切换日志一致：`ip` + 转发头）。

| 取值 | 行为 |
| --- | --- |
| `true`（默认） | 写操作也要求来源在 `server.permissionAllowlist` 内 —— 密钥泄漏后非信任来源也写不进来 |
| `false` | 退回「只看摘要」的旧行为，适合必须从公网 IP 写入的自动化脚本 |

- 开关可用环境变量 `INDEX_SRV_DIGEST_ALLOWLIST`（`0/false/no/off` 视为关闭）覆盖，方便容器部署；
- **提升接口不受这个开关影响**：`POST /api/permission` 始终受白名单约束（上面的表格），关掉开关只放宽写操作；
- 读取类接口（`GET /api/*`）从不做网段限制。
...
- 若来源处在**可信网段免密钥**模式（[§2](#2-鉴权与权限级别)），`GET` / `POST` 都直接返回 `level: 0` / `reason: "trusted-network"` —— `POST` 连请求体都不需要（客户端在 http 下根本算不出摘要）

### 可信网段免密钥（`server.trustedNetworkBypass`）

浏览器只在**安全上下文**（`https` 或 `localhost`）提供 Web Crypto，所以通过 `http://内网IP` 访问时客户端**算不出摘要**，会直接拒绝提权（连请求都不发）。若该网段本身可信（例如自家的 WireGuard 内网），可开启这条通道：

| 取值 | 行为 |
| --- | --- |
| `false`（默认） | 凭据只有密钥：白名单内的来源也必须提交有效摘要 |
| `true` | 白名单内的来源**免摘要**即 `level: 0`、`reason: trusted-network`，提权与写入都放行 |

- **凭据由密钥换成来源网段**：开启后白名单内任何设备都可写，`INDEX_SRV_SECRET` 对该网段不再起作用 —— 只在内网确实可信时开启；可用环境变量 `INDEX_SRV_TRUSTED_BYPASS` 覆盖；
- **安全前提不变**：服务端未配置密钥时仍然只读（`3 / unconfigured`），免密钥通道不会把「未配置密钥」变成可写；
- **可见性**：开启时启动日志有一条 `warn「可信网段免密钥已启用…」`，每次借该通道提权记一条 `权限切换 3 user -> 0 super pass`（`reason: trusted-network`），写操作记 `debug` 级留痕；
- 白名单**之外**的来源不受影响：提权与写入依旧 `403`。
